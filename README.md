...
每天达拉斯时间零点，[rebasdaily.com](https://rebasdaily.com) 自动出一期新刊：**学术 / 开源 / 科技 / 数据 / 商业 / 量化 / 设计 / 艺术** 八个板块，150+ 信息源的数百条候选被蒸馏成 60~80 篇中文报道——选题、交叉核查、背景铺垫、撰写、排版、部署，全程无人参与。

信息源是**国际视野**的：从 arXiv、Nature/Science 等顶刊、Bloomberg/WSJ 等财经大刊的标题层、硅谷公司博客，到中国 AI 媒体（DeepSeek 生态）、日欧设计杂志与国际设计奖项，再到 Reddit 日榜与 X 上的行业人物——中英日多语言、网页/API/社交媒体多形态进同一条管线（下游 LLM 语言无关），统一产出中文刊物。
...
flowchart LR
    subgraph 采集层["采集层（代码，非 LLM）"]
        A[150+ 信息源<br>RSS/API/网页/社交媒体] --> B[关键词预筛<br>规范化去重] --> C[(SQLite 候选池<br>+30 天顶刊池)]
    end
...
| **主编** | 选题、事件聚类、定版面 | 材料深度决定篇幅；宁缺毋滥不凑数；thread_key 事件线跨日去重；收尾批**补充轮**给薄板块用当日新候选补选；顶刊池候选跨期可选、不算旧闻；**《经典鉴赏》栏目策展**（艺术板块每日一件经典作品：周一至五油画、周末自由门类，联网核实+Wikipedia 直图闸门，拿不到可用图就换作品重提） |
| **背调** | 给选题配背景材料 | 双模式：技术板块=概念解释，人文板块=背景故事；可查阅 30 天往期做连续报道衔接；**薄材料联网调查**——只有标题的新闻/repo 选题、以及**原文实在拿不到的论文**（付费墙顶刊无预印本）由背调联网搜索补充事实细节（带来源、经审核、撰写时按"据 X 报道"归因） |
| **核查** | 簇内交叉印证 + 审校背调产物 | 独立信源计数定可信度；背景逐条 ok/fix/drop，宁删勿留 |
| **撰写** | 专题长文 + 速览短报道 | **防幻觉铁律**：事实只认供稿材料，背景只认审核后的背景块；**论文类专题与速览都抓 arXiv 原文精读**（速览用较小上限保持简短、只挑最有价值的一点；顶刊 DOI 论文借库内同题 arXiv 预印本兜底；原文用完即删不进库）；**兼任图片编辑**（艺术/设计板块）：候选图以多模态附件实际过目——低质/无关的删（可全弃走无图版式）、保留图排序定头图、在正文最相关的段落后插图并配图注 |
| **文风** | `style.md` 单点调节全刊语域 | "给聪明但不搞技术的朋友讲最新进展"：准确、松弛、克制；术语宁多勿少地解释，复杂工作用生活化例子开场 |

## 值得参考的设计决策

- **防幻觉的双材料制**——writer 被禁止使用自身知识：报道事实只能来自供稿材料，领域背景只能来自"背调 agent 产出 + 核查 agent 审校"的背景块。两条供给线各有独立纪律，写手专心写作。唯一受控例外是**调查补充**：仅标题级的新闻/repo 选题、以及原文拿不到的论文（付费墙顶刊无预印本，检索期刊新闻稿/出版方页面）由背调联网检索补事实（红线=只写检索结果、禁凭记忆），审核按新闻口径过一遍，成稿以"据 X 报道"归因——付费墙大刊的标题层和 X 上的只言片语因此变成可用信源。
...
- **零 JS 前端**——纯静态 HTML（CSS 全内联，file:// 直开可用），数学公式在**构建期** LaTeX→MathML（浏览器原生渲染，不引 KaTeX）；PWA 的 Service Worker 只做离线兜底，页面渲染不依赖 JS（仅有的两处渐进增强：SW 注册与移动端分享键）。艺术/设计板块**多图排版**：采集期从 feed 正文提取图库（噪声过滤 + 同图多尺寸去重），头条/专题卡/报道页按图量自适应单图或杂志式多图版式；撰写 agent 兼任图片编辑（多模态实看候选图），审选结果与正文插图位置由它裁决——机器抓的图不再盲目上刊。
- **采集层用 urllib 而非 httpx**——Cloudflare 按 TLS 指纹拦截，httpx 全军覆没、urllib 全源畅通。管线里最省钱的一行经验（反例也遇到过：个别站拦 urllib 放 curl——新源冒烟必须用生产同款客户端）。
- **双车道采集**——Reddit/X 镜像这类按 IP 严格限速的源（实测 ~1 请求/分钟）标记 `pace_seconds` 进慢车道：独立 cron 每小时串行滴灌、源间隔限速、429 退避重试；常规源照旧 8 线程并发。候选殊途同归入池，出刊管线零感知。
