...
def test_render_site(tmp_path):
    """最小数据集渲染：issue/报道/归档三类页面 + 编辑要求。

    2026-07 评审后规则：核查记录不上前端；速览有报道时链内页，无报道（旧数据）回退外链。
    """
    import dataclasses
    import json
    ...
    from rebas import db as database
    from rebas.config import load_config
    from rebas.render.site import render_site
    ...
    conn.execute(
        ...
        " VALUES ('hn-frontpage','academic','http://orig.example/a','http://orig.example/a',"
        " '速览原文','2026-01-01')")
    ...
    conn.execute(
        ...
        " ?, '理由')",
        (json.dumps([item_id]),
         json.dumps({"claims": [{"claim": "论断A", "support": 1, "confidence": "single"}],
                     "notes": "备注"})))
    ...
    stats = render_site(conn, conf, "2026-01-01")
    assert stats["features"] == 1 and stats["briefs"] == 2

    issue = (tmp_path / "site" / "issue-2026-01-01.html").read_text()
    assert "topic-2026-01-01-brief-key.html" in issue  # 有报道的速览 → 内页
    assert "http://orig.example/a" in issue            # 无报道的旧速览 → 原文外链兜底
    assert "card-fallback" in issue                    # 无图兜底
    topic = (tmp_path / "site" / "topic-2026-01-01-test-topic.html").read_text()
    assert "供稿材料" in topic                          # 来源列表保留
    assert "单一信源" not in topic and "核查记录" not in topic  # 核查记录不上前端
    assert "<h2>小标题</h2>" in topic                   # markdown 渲染
    brief_page = (tmp_path / "site" / "topic-2026-01-01-brief-key.html").read_text()
    assert "速览正文" in brief_page and "供稿材料" in brief_page and "· 速览" in brief_page
    assert (tmp_path / "site" / "archive.html").exists()
    assert (tmp_path / "site" / "index.html").exists()
    assert (tmp_path / "site" / "static" / "style.css").exists()
