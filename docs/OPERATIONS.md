...
- `config/config.toml` —— 全局配置（llm 后端与角色型号、出刊参数；`site_keep_days=7` 往期完整页面保留一周，更早进归档存目——渲染期策略，调大重渲染可找回；`classic_board="art"` =《经典鉴赏》栏目：该板块每日一件经典作品，周一至五油画/周末自由，classic 角色联网提名 + Wikipedia 直图闸门，拿不到图换作品重提最多 3 次，""=关闭；**防重复（2026-09-02/03 定稿，清单导流+规则保证双层）**：与规则同源同窗的已鉴赏清单喂给提名模板做导流（省盲提撞窗的试错调用）；`classic_dedupe_days`（缺省 180，admin 可调）滑窗内艺术+设计两栏目合并的身份集 × 四条规则——作品名归一键 / thread_key / 原始 wiki URL / 重定向解析后的规范 wiki URL（换译名、别名、条目名都认得出来），命中即退回重提（上限 5 次）；窗口外允许再登；`classic_reset_dates = { 板块 = "日期" }` = 栏目系列重置：该板块早于此日期的栏目选题不计入身份集（历史"清零"允许重做旧选题，数据零改动、删键即恢复；按 topic 板块判定，另一栏目引用过的作品不受影响）；栏目规则文本在 `config/prompts/editor_classic.md` 可编辑；`classic_design_board="design"` = 设计版《经典鉴赏》（2026-08-05）：同款提名与图片闸门，门类按星期轮换（产品/建筑/平面/家具/交通工具、周末自由），规则文本在 `config/prompts/editor_classic_design.md`，""=关闭；两版鉴赏的撰写走导览员专用模板（`writer_classic_art.md` / `writer_classic_design.md`，不注入新闻向 style.md）；`classic_paper_board="academic"` =《经典重读》栏目：淡日供给——收尾批（--refill）该板块零专题时，classic 角色提名一篇突破性经典论文，闸门=arXiv API 题录核对 + 真实抓到全文（落 paper_cache，fetch 不双抓），拿不到换一篇最多 3 次，成题即淡日头条、全文精读，""=关闭；规则文本在 `config/prompts/editor_classic_paper.md`）
...
- `config/prompts/*.md` —— agent 提示词模板（string.Template `$` 占位符，改文风不动代码）；**style.md = 全刊文风单点调节**；researcher*.md = 背景调查（含 30 天往期查阅两轮协议 + **调查补充**：仅标题级的新闻/repo 选题、以及原文精读拿不到的论文（付费墙顶刊无预印本，以取材期缓存为准）标注【需调查补充】，背调联网搜索补事实细节，`research_facts_max=0` 关闭；搜索走 `[llm] search_roles`，须写在 `[llm.roles]` 表头之前）；researcher_story.md = 背调第二条 lane（**故事素材**：经典鉴赏题恒查、新闻专题也查，速览与论文题不查，联网搜轶事/来龙去脉/文化回响，`research_stories_max=0` 关闭）；**相关盘口**（2026-09-02，facts lane 内）= 新闻/时效类选题标注【可配盘口】，agent 从刊内盘口快照池（Polymarket/Kalshi 近 36h 条目按交易量取前 24）挑直接相关的盘当辅助信源——只回池内编号+一句关联，赔率快照由代码逐字回填不过背景审核，`research_odds_max=0` 关闭、`research_odds_boards` 定注入板块；checker_background.md = 背景审核（概念宁删勿留，facts 按新闻口径放宽，stories 做 fact check + 策展——大原则=精彩度，离题但精彩可留，又不相关又平淡才删，**宁留勿删**）
...
批4/批5 收尾后各做一次 `render` + 部署（零 token，内容受发布闸门保护与当日一致）——**白天同步的前端改动当天上线**，不用等次日批1翻牌；批1 的部署职责不变（翻牌=内容更新）。
...
测试：`.venv/bin/pytest -q tests/`（149 项）。
